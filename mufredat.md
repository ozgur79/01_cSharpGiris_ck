...
|------|----------------------|-------------------|----------------|----------------------|-----|
| `using System;` / `using System.Collections.Generic;` / `using System.Linq;` / `using System.Text;` / `using System.Threading.Tasks;` | ck0010 | **açıldı: `02_cSharpForm_ck/ck1280_usingKapaniyor`** | 3 tur — **emekli**, açılışı da tamam | ck0020: hazır kütüphaneleri projene dahil eder; ck0080: neden 5 ayrı satır var, her biri ayrı bir "araç kutusu" (2. tur, yeni açı); ck0160: hepsi her programda kullanılmaz, bu derste sadece System işe yaradı (3. tur, yeni açı); ck1280: `using` satırı silinip derlenerek NEDEN gerekli olduğu deneyle gösterildi (açılış) | [iskelet-bağımlı] — VS'in her yeni Console projesine otomatik eklediği satırlar |
| `namespace <proje adın>` | ck0010 | **açıldı: `02_cSharpForm_ck/ck1260_iskeletKapaniyor`** | 3 tur — **emekli**, açılışı da tamam | ck0030: kodları isim altında gruplayan "klasör" gibidir, proje adından VS'in ürettiği isim; ck0090: her yeni projede namespace'in neden değiştiği (2. tur, yeni açı); ck0170: her yeni proje kendi namespace'ini alır (3. tur, yeni açı); ck1260: Program.cs okunarak namespace'in "tüm sınıfları toplayan çatı" işlevi açıklandı (açılış) | [iskelet-bağımlı] |
| `class Program` | ck0010 | **açıldı: `02_cSharpForm_ck/ck1260_iskeletKapaniyor`** | 3 tur — **emekli**, açılışı da tamam | ck0040: namespace'in içindeki kutulardan biri, yazdığın her satır bu kutuda durdu; ck0110: adı hep aynı kalır (VS otomatik verir), namespace proje adına göre değişir (2. tur, yeni açı); ck0220: kaç satır yazarsan yaz kutu büyümüyor, sadece içi doluyor (3. tur, yeni açı); ck1260: `static class Program`ın hiç nesne üretilmeyen bir sınıf olduğu, ck1270'teki `class Ogrenci` ile karşıtlığı açıklandı (açılış) | [iskelet-bağımlı] |
| `static void Main(string[] args)` | ck0010 | **açıldı: `02_cSharpForm_ck/ck1260_iskeletKapaniyor`** | 3 tur — **emekli**, açılışı da tamam | ck0050: class Program'ın içinde, programın başladığı satır; ck0120: başladığı satırdan itibaren kod yukarıdan aşağı sırayla çalışır (2. tur, yeni açı); ck0250: iki farklı yazım (ck0240/ck0250) aynı işi yapsa da ikisi de yine bu satırdan başladı — yazım değişir, başlangıç değişmez (3. tur, yeni açı); ck1260: WinForms'ta aynı kutunun `Application.Run(new Form1());` çağırdığı gösterildi (açılış) | [iskelet-bağımlı] |
| `Console.ReadKey();` | ck0010 | **açıldı: `02_cSharpForm_ck/ck1260_iskeletKapaniyor`** | 3 tur — **emekli**, açılışı da tamam | ck0060: Main'den sonraki son satır, o olmasa pencere anında kapanır; ck0130: iki sonuç hesaplanan derste neden en sonda durması gerektiği (2. tur, yeni açı); ck0270: if/else'in sadece bir dalı çalışsa da hangi yoldan gidilirse gidilsin en sonunda hep bu satıra gelinir (3. tur, yeni açı — kapanış); ck1260: WinForms'ta bu satırın neden hiç GEREKMEDİĞİ açıklandı — `Application.Run` formu açık tuttuğu sürece program zaten kapanmıyor (açılış, karşıt örnekle) | rotasyona ck0060'ta katıldı |
...
| `new` (nesne oluşturma) | ck0530 | **açıldı: `02_cSharpForm_ck` ünite 07 (ck1200/ck1210/ck1230/ck1270)** | rotasyon dışı, tekrar etti | ck0530: Random tarifinden gerçek bir üretici oluşturmak için kullanılıyor, kendi tariflerimizi (sınıf) yazınca daha sık göreceğiz; ck1200: kontrollerin de `new` ile üretildiği Designer.cs'te gösterildi; ck1230: aynı sınıftan (Button) çok nesne üretilince her birinin bağımsız olduğu görüldü; ck1270: öğrenci kendi sınıfını (`class Ogrenci`) yazıp `new Ogrenci()` ile nesne üretti (kapanış — artık BAŞKASININ değil KENDİ sınıfından nesne üretiyor) | 5 iskelet kutusundan farklı, ayrı takip edilen bir kutuydu — döngüsü burada kapandı |
